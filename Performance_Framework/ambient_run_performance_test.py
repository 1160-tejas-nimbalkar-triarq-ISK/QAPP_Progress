#!/usr/bin/env python3
"""
Ambient API Performance Testing Framework
A Locust-based performance testing framework for Ambient APIs

This script provides a simplified interface for running performance tests
against Ambient APIs using Locust load testing tool.
...
class AmbientPerformanceTestRunner:
    ...
    def __init__(self, config_file=None, environment=None):
        """Initialize the Ambient performance test runner"""
        ...
        print(f"🚀 Ambient Performance Testing Framework Initialized")
    ...
    def list_available_tests(self):
        ...
        print("\n🔍 Available Ambient Test Configurations:")
        print("=" * 50)
    ...
    def run_locust_test(self, test_config_name="light_load", environment=None, custom_config=None):
        ...
        print(f"🦗 Running Ambient Locust Performance Test: {test_config_name}")
        ...
        if custom_config:
            ...
            try:
                ...
                if test_config_name in load_tests:
                    ...
                    config['endpoint'] = '/Ambient/generate_summary_html'
                    config['endpoint_name'] = 'original'
                    config['endpoint_description'] = 'Original Ambient API'
        ...
        try:
            ...
            endpoint_path = config.get('endpoint', '/Ambient/generate_summary_html')
            endpoint_name = config.get('endpoint_name', 'original')
            endpoint_description = config.get('endpoint_description', 'Original Ambient API')
        ...
        endpoint_suffix = f"_{endpoint_name}" if endpoint_name != "original" else ""
        html_report = f"ambient_performance_report_{environment}_{config['users']}users{endpoint_suffix}_{timestamp}.html"
        report_path = self.reports_dir / html_report
        
        # Locust file path
        locust_file = self.tests_dir / "ambient_api_locust.py"
        
        # Check if locust file exists
        if not locust_file.exists():
            print(f"❌ Locust file not found: {locust_file}")
            print("💡 Make sure ambient_api_locust.py exists in the tests directory")
            return False, None
    ...
    def generate_pdf_report(self, html_report_path, environment, config):
        ...
        try:
            ...
            pdf_path = generate_pdf_from_html(
                ...
                environment, 
                config['users'],
                config.get('description', 'Ambient Performance Test')
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Ambient Performance Testing Framework',
...
Examples:
  ...
  %(prog)s --list
    ...
    parser.add_argument('--test', '-t',
                        choices=['light_load', 'medium_load', 'heavy_load', 'light_load_v1', 'medium_load_v1', 'heavy_load_v1', 'stress_test', 'stress_test_v1', 'endurance_test', 'endurance_test_v1', 'spike_test'],
                        default='light_load',
    ...
    try:
        ...
        runner = AmbientPerformanceTestRunner(
        ...
        # Run the test
        print(f"🎯 Starting Ambient performance test...")
