...
public class ReleasedTask {
  ...
  @Test(priority=6,dependsOnMethods = "mylogin", enabled=true)
    public void Prcaticefilter2() throws InterruptedException {
    	 //select filter:
    	 //select practice filter
    	 WebElement dropdown2 = driver.findElement(By.xpath("(//span[contains(.,'All')])[2]"));
    	 dropdown2.click();
...
//        	  WebElement dropdown_project = driver.findElement(By.xpath("//input[@type='text']"));
    ...
    @Test(priority=9,dependsOnMethods = "mylogin",enabled=false)
    ...
    @Test(priority=10,dependsOnMethods = "mylogin",enabled=false)
...
//        wait.until(ExpectedConditions.urlContains("/evreleasetasks"));
    	...
    	try {
    		   WebElement assignedToDropdown = driver.findElement(By.xpath("(//div[contains(.,'Ujwala Gavit')])[24]"));
